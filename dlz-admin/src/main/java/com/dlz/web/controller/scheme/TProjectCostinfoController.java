...
public class TProjectCostinfoController extends BaseController
{
     ...
     */
    @RequiresPermissions("scheme:costinfo:edit")
    @Log(title = "方案成本数据", businessType = BusinessType.INSERT)
    @PostMapping("/importCostData")
    @ResponseBody
    public AjaxResult importCostData(@RequestParam("file") MultipartFile file, @RequestParam("faid") Long faid) throws IOException {

        return toAjax(tProjectCostinfoService.importExcelData(file,faid));
    }
}
